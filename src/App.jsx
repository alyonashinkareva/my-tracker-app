    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans text-stone-900">
      ...
      `}>
        <div className="p-8 cursor-pointer" onClick={() => {setActiveTab('home'); setIsSidebarOpen(false);}}>
          <h1 className="text-3xl font-black text-[#3D2B1F] flex items-center gap-2 tracking-tighter uppercase transition-transform active:scale-95">
          ...
          {menuItems.map(item => (
            <button key={item.id} onClick={() => { setActiveTab(item.id); setIsSidebarOpen(false); }}
              className={`w-full flex items-center gap-4 px-6 py-4 rounded-3xl font-black transition-all
                ${activeTab === item.id ? 'bg-[#3D2B1F] text-white shadow-xl shadow-stone-200 scale-[1.02]' : 'text-stone-400 hover:bg-stone-50 hover:text-[#3D2B1F]'}`}>
        ...
        <div className="p-8 border-t border-stone-50">
          <div className="flex items-center gap-3 p-3 bg-stone-50 rounded-2xl">
            <div className="w-10 h-10 bg-[#3D2B1F] rounded-xl flex items-center justify-center font-black text-white text-xs uppercase">
